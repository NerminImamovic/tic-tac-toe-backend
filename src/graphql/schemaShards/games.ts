...
export default {
  resolvers: {
    ...
    Game: {
      Player1: async (game: Partial<GQL.Game>) => await userService.getPublicUser(game.player1Id),
      Player2: async (game: Partial<GQL.Game>) => await userService.getPublicUser(game.player2Id),
      NextTurnPlayer: async (game: Partial<GQL.Game>) => await userService.getPublicUser(game.nextTurnPlayerId),
      Winner: async (game: Partial<GQL.Game>) => await (await userService.getPublicUser(game.winnerId)),
      Moves: async (game: Partial<GQL.Game>) => await ticTAcToeGameService.getMovesForGames(game.id),
    },
  },
  typeDefs: [typeDefs],
};
